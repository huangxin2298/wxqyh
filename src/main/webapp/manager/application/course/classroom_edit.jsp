        ...
        <form action="" method="post" id="classroomForm" onsubmit="return false;"class="form new_form">
            <div class="form-item">
                ...
                <div class="form-field">
                    <input type="text" name="classroomName" placeholder="课室名" id="classroomName" class="form-text input-text" readonly>
                </div>
            </div>
            <div class="form-item">
                <div class="title">
            ...
            <div class="form-item">
                ...
                <div class="form-field">
                    <input id="ismultime0" type="radio" name="ismultime" value="0">否</input>
...
<script>
    ...
    function getClassroomInfo() {
        $.ajax({
            url: "${baseURL}/manager/course/getClassroomInfo.action",
            type: "GET",
            async: false,
            data: {"classroomId":"${param.classroomId}"},
            dataType: "json",
            success: function (result) {
                if (result.code == "0") {
                    var classroomInfo = result.data;
                    if(typeof (classroomInfo)!="undefined" && classroomInfo!=null){
                        $("#classroomId").val(classroomInfo.id);
                        $("#classroomName").val(classroomInfo.classroomName);
                        $("#admissNum").val(classroomInfo.admissNum);
                        $("#address").val(classroomInfo.address);
                        $("#ismultime"+classroomInfo.ismultime).attr("checked","checked");
                    }
                } else {
    ...
    function publish(){
        ...
        $.ajax({
            url: "${baseURL}/manager/course/updateClassroomInfo.action",
            type: "POST",
            data: form,
            async: false,
            dataType: "json",
