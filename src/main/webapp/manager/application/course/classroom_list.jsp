...
<form action="" method="post" id="searchForm" onsubmit="return false;">
    <div class="topMainBar clearfix">
        <div class="fl flMainBar" style="margin-top: 6px;">
            <a id="" href="javascript:addClassroom();" onclick="" class="btn orangeBtn smallBtn">新增</a>
        </div>
        <%--<a class="senior_search_btn">高级搜索</a>--%>
        <div class="fr">
            <div class="searchBox">
                <input type="hidden" name="curPage" id="curPage">
                <input placeholder="课室名" class="inputSearchBox" type="text" name="inputSearch" id="inputSearch"/>
                <input class="submitSearchBtn" type="button" onclick="getClassroomList()" value=""/>
            </div>
        </div>
    </div>
</form>
<div class="contactTable mt10" id="cc94a9ec68ff4d93a7fd5934c581b647">
    <table class="table table-line table-hover">
        ...
        <tr>
            ...
            <th width='200'>课室名</th>
            <th width="100">可容纳人数</th>
            <th width='200'>地址</th>
            <th width="100">是否有多媒体</th>
            <th width="100">状态</th>
            <th width="100">操作</th>
        </tr>
        </thead>
...
<script>
    ...
    var classroomTemplate = "<tr>" +
        ...
        "                        <a href=\"javascript:updateClassroomStatus('@{id}','@ClassroomStatus')\">@oper</a>" +
    ...
    function getClassroomPage(curPage) {

        $("#pageData").html("");
        $("#curPage").val(curPage);
        var form = $("#searchForm").serialize();
        $.ajax({
            url: "${baseURL}/manager/course/getClassroomPage.action",
            type: "POST",
            async: false,
            data: form,
            dataType: "json",
                    ...
                    if(typeof (pageData)!="undefined" && pageData!=null){
                        if(parseInt(pageData.totalRows)>0){
                            ...
                            if(pageData.curPage <= 1){
                                ...
                                $("#lastPage").children("a").attr("href","javascript:getClassroomPage(\""+(parseInt(pageData.curPage) - 1)+"\")");
                            }
                            if(pageData.curPage >= pageData.totalPages){
                                $("#nextPage").children("a").attr("href","javascript:void(0)");
                            }else{
                                $("#nextPage").children("a").attr("href","javascript:getClassroomPage(\""+(parseInt(pageData.curPage) + 1)+"\")");
                            }
                            var classroomPage = pageData.data;
                            if(typeof(classroomPage)!="undefined" && classroomPage != null && classroomPage.length>0){
                                var pageDateHtml = "";
                                for(var i=0;i<classroomPage.length;i++){
                                    var classroomHtml = classroomTemplate;
                                    classroomHtml = classroomHtml.replace(/@{id}/g,classroomPage[i].id);
                                    classroomHtml = classroomHtml.replace("@classroomName",classroomPage[i].classroomName);
                                    classroomHtml = classroomHtml.replace("@admissNum",classroomPage[i].admissNum);
                                    classroomHtml = classroomHtml.replace("@address",classroomPage[i].address);
                                    var ismultime = "";
                                    if(classroomPage[i].ismultime=="0"){
                                        ismultime = "否";
                                    }else if(classroomPage[i].ismultime=="1"){
                                        ismultime = "是";
                                    }
                                    classroomHtml = classroomHtml.replace("@ismultime",ismultime);
                                    var status = "";
                                    if(classroomPage[i].status=="0"){
                                        status = "停用";
                                        classroomHtml = classroomHtml.replace("@ClassroomStatus","1");
                                        classroomHtml = classroomHtml.replace("@oper","启用");
                                    }else if(classroomPage[i].status=="1"){
                                        status = "启用";
                                        classroomHtml = classroomHtml.replace("@ClassroomStatus","0");
                                        classroomHtml = classroomHtml.replace("@oper","停用");
                                    }
                                    classroomHtml = classroomHtml.replace("@status",status);
                                    pageDateHtml += classroomHtml;
                                }
    ...
    function updateClassroomStatus(classroomId,status){
        $.ajax({
            url: "${baseURL}/manager/course/updateClassroomStatus.action",
            type: "POST",
            data: {"classroomId":classroomId,"status":status},
            dataType: "json",
            success: function(result) {
                if(result.code == "0"){
                    alert(result.describe);
    ...
    function doEdit(classroomId){
        document.location.href = "${baseURL}/manager/application/course/classroom_edit.jsp?classroomId="+classroomId;
    }
    function doDel(classroomId){
        if(!confirm("您确定要删除该课室？")){
            return;
        }
        $.ajax({
            url: "${baseURL}/manager/course/delClassroomById.action",
            type: "POST",
    ...
    function addClassroom(){
        document.location.href = "${baseURL}/manager/application/course/classroom_add.jsp";
    }
    function loadOpt(){
        $('.lasttd').on({
            mouseover: function () {
